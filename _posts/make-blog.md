...
블로그를 직접 만든다고 생각하면 덜컥 겁이 났습니다.
Next 프로젝트에서 Tiptap Editor를 활용했지만 커스터마이징이 쉽지 않았던 경험이 있었습니다.
이미지 파일을 많이 저장하게 되면 AWS를 써야 하나? Docker로 서버를 띄울까? 서버 컴퓨터는 휴대폰 공기계로 해놔야 할까? 만약 DB가 날라가면 내가 작성한 모든 글이 날라가게 되는 거 아닌가? 등 여러 걱정이 들었습니다.

구글에 Next를 이용하여 블로그를 제작한 글들을 보니 vscode에서 직접 마크다운 파일을 만드는 방식이 많았습니다.
...
저는 최대한 간결한 디자인을 원했고 언젠가 꼭 한 번 일해보고 싶은 토스의 기술 블로그를 참고하여 화면을 구성했습니다.
...
추출된 포스팅 정보입니다. 하나의 객체에 작성한 정보들이 요소로 들어온 걸 확인할 수 있습니다.
